...
void PurePursuit::createControlCommand() {
    ...
    // erro lateral
        ...
        if(velocidade<0.7){ajuste_lateral = 0;} // evitar erros quando carro está parado, lim(v->0) 1/(velocidade) = inf
    ...
    // Speed Controller
        ...
        double vref;
        if(i_center_line > 960 | i_center_line < 160){
            vref = 6;
        }
        else{
            vref = 4;
        }
        control_command_.throttle.data = static_cast<float>(kappaP*(vref - vel));
    }
