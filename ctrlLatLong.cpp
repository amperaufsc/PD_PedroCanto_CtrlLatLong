...
void PurePursuit::createControlCommand() {
    ...
    // Steering Control Pedro de Bem
    {
    // algumas constantes
        double kappa = 1.0;
        int sign;
        double tangent_x = center_line_.points[i_center_line+1+2].x - center_line_.points[i_center_line-1+2].x;
        double tangent_y = center_line_.points[i_center_line+1+2].y - center_line_.points[i_center_line-1+2].y;
    ...
    // erro lateral
        ...
        if(velocidade<0.5){ajuste_lateral = 0;} // evitar erros quando carro está parado, lim(v->0) atan(1/velocidade) 
                                                // cresce independente do erro lateral
    
    // sinal de controle    [-pi/2 , pi/2]    negativo:->   positivo:<-
        control_command_.steering_angle.data = 0.2*static_cast<float>(angular_error+ajuste_lateral)
                                             + 0.8*control_command_.steering_angle.data;
    }
    { // Speed Controller
